...
// ==========================================
// 1. دالة سحب المقال والصور باحترافية
// ==========================================
async function scrapeArticle(url) {
    try {
        const response = await axios.get(url, {
            ...
            maxRedirects: 10,
            timeout: 15000 // إنهاء المحاولة إذا تأخر الموقع أكثر من 15 ثانية
        });
        ...
        return {
            title: article.title,
            rawText: article.textContent.trim().slice(0, 4500),
            images: [...new Set(images)], 
            originalLink: response.request.res.responseUrl
        };
    } catch (error) {
        // عدم طباعة الخطأ بالكامل لتقليل الإزعاج في الشاشة، فقط نعيد null
        return null; 
    }
}
...
async function formatWithAI(title, text) {
    try {
        console.log("🤖 جاري صياغة المقال بأسلوب صحفي جذاب...");
...
// ==========================================
// 3. المحرك الرئيسي (مع نظام المحاولات المتعددة)
// ==========================================
async function runGoogleNewsBot() {
    try {
        ...
        // خلط أول 15 خبر لضمان التنوع
        let articles = feed.items.slice(0, 15).sort(() => 0.5 - Math.random());
        let publishedSuccessfully = false;

        // حلقة التكرار: محاولة سحب الأخبار واحداً تلو الآخر حتى ينجح واحد
        for (const item of articles) {
            console.log(`\n📡 محاولة سحب الخبر: ${item.title}`);
            const articleData = await scrapeArticle(item.link);
            
            if (!articleData || articleData.rawText.length < 500) {
                console.log("⏭️ محتوى ضعيف أو محمي، جاري القفز للخبر التالي...");
                continue; // الذهاب للخبر التالي فوراً
            }

            console.log("✅ تم سحب المحتوى بنجاح! جاري المعالجة...");
            ...
            if (articleData.images.length > 1) {
                ...
                for (let i = 1; i < Math.min(articleData.images.length, 5); i++) {
                    galleryHtml += `<img src="${articleData.images[i]}" style="width: 100%; height: 160px; object-fit: cover; border-radius: 12px; box-shadow: 0 4px 10px rgba(0,0,0,0.1);"/>`;
                }
                galleryHtml += `</div>`;
            }
            ...
            const beautifulPostHtml = `
                <div dir="ltr" style="background-color: #ffffff; padding: 30px; border-radius: 24px; box-shadow: 0 10px 40px rgba(0,0,0,0.08); font-family: 'Segoe UI', system-ui, -apple-system, sans-serif; max-width: 800px; margin: 0 auto; color: #2d3748; overflow: hidden;">
                    ...
                    <div style="position: relative; margin-bottom: 30px; border-radius: 16px; overflow: hidden; box-shadow: 0 8px 25px rgba(0,0,0,0.15);">
                        ...
                        <div style="position: absolute; bottom: 15px; left: 15px; background: rgba(0,0,0,0.8); color: #fff; padding: 6px 14px; border-radius: 20px; font-size: 13px; font-weight: bold; backdrop-filter: blur(5px);">
                            🔥 TRENDING NOW
                        </div>
                    </div>
                    ...
                    <div style="line-height: 1.85; font-size: 1.1rem;">
                        <style>
                            ...
                            p:first-of-type { font-size: 1.25rem; font-weight: 500; color: #2d3748; letter-spacing: -0.01em; border-bottom: 1px solid #edf2f7; padding-bottom: 15px; }
                        </style>
                        ${finalContent}
                    </div>
                    ...
                    <div style="margin-top: 50px; padding-top: 20px; border-top: 2px dashed #e2e8f0; display: flex; justify-content: space-between; align-items: center; font-size: 0.9rem; color: #718096;">
                        ...
                        <a href="${articleData.originalLink}" target="_blank" style="color: #3b82f6; text-decoration: none; font-weight: 600; padding: 8px 16px; background: #ebf8ff; border-radius: 20px;">Read Original ↗</a>
                    </div>
                </div>
            `;
            ...
            await blogger.posts.insert({
                ...
                requestBody: {
                    ...
                    labels: ['Google News', 'Tech Trends']
                },
                isDraft: false
            });

            console.log(`\n🎉 تم النشر بنجاح على مدونتك! عنوان المقال: ${articleData.title}`);
            publishedSuccessfully = true;
            break; // الخروج من حلقة التكرار لأننا نشرنا مقالاً بنجاح
        }

        if (!publishedSuccessfully) {
            console.log("\n❌ تم فحص جميع الأخبار ولم نتمكن من سحب أي محتوى دسم، سيتم المحاولة في الدورة القادمة.");
        }
